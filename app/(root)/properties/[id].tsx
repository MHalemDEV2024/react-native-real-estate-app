 ...
 * -----------------------------------------------------------------------------
 * Shows:
 *  - Property hero image with header actions
 *  - Property details (title, type, rating, features)
 *  - Agent info
 *  - Overview, facilities, gallery, location
 *  - Reviews (first one preview)
 *  - Bottom booking bar with price + CTA
 *
 * Tech Stack:
 *  - React Native + Expo Router
 *  - Tailwind (via nativewind) for styling
 *  - FlatList for gallery rendering
 */
const Property = () => {
      ...
      >
        ...
        <View className="px-5 mt-7 flex gap-2">
          ...
          <View className="flex flex-row items-center gap-3">
            ...
            <View className="flex flex-row items-center gap-2">
              ...
              <Text className="text-black-200 text-sm mt-1 font-rubik-medium">
                {(property as any)?.rating} ({(property as any)?.reviews?.length ?? 0} reviews)
              </Text>
            </View>
          </View>
          ...
          <View className="mt-7">
            ...
            {((property as any)?.facilities?.length ?? 0) > 0 && (
              <View className="flex flex-row flex-wrap items-start justify-start mt-2 gap-5">
                {((property as any)?.facilities ?? []).map((item: string, index: number) => {
                  // Find facility in our local constant
          ...
          {/* Reviews */}
          {/* --------------------------- */}
          {((property as any)?.reviews?.length ?? 0) > 0 && (
            <View className="mt-7">
              <View className="flex flex-row items-center justify-between">
                <View className="flex flex-row items-center">
                  <Image source={icons.star} className="w-6 h-6" />
                  <Text className="text-black-300 text-xl font-rubik-bold ml-2">
                    {(property as any)?.rating ?? 0} ({(property as any)?.reviews?.length} reviews)
                  </Text>
                </View>
